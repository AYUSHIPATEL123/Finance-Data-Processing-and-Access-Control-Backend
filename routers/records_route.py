...
async def records(db:Annotated[AsyncSession,Depends(get_db)]):
    query = select(Record)
...
async def add_record(data:RecordSchema,db:Annotated[AsyncSession,Depends(get_db)]):
...
async def update_record(id:int,new_data:RecordSchema,db:Annotated[AsyncSession,Depends(get_db)]):
...
async def del_record(id:int,db:Annotated[AsyncSession,Depends(get_db)]):
    record = await db.get(Record,id) 
    if not record:
